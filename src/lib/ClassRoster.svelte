<!-- ClassRoster.svelte: whole-class exercise, reflection, debate, and story
     tabs. The Exercises tab is a bare name + overall score table (a pass on
     a type counts if none of that student's answers of it were wrong, a
     correct-after-retry included). -->
<script lang="ts">
  ...
  $: reflectionGroups = REFLECTION_CONCEPTS.map(({ key, test, idea, counts }) => {
    ...
    for (const s of students) {
      ...
      if (turns.length && test(turns.join(" "))) mentions.push({ name: s.name });
    }
    return { key, idea, counts, mentions };
  });
  ...
  $: debateRows = students.map((s) => {
    const { winner } = debatePicks(s, logicalSideName);
    return { name: s.name, winner };
  });
